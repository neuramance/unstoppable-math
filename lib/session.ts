...
function splitSessions(log: SessionLog): { plan: SessionPlan; trials: Trial[] }[] {
  ...
  for (const ev of log) {
    if (ev.kind === 'start') sessions.push({ plan: ev.plan, trials: [] })
      ...
      if (!open) throw new Error('trial before any session start')
...
export function replayLog(lesson: Lesson, log: SessionLog): LogAudit {
  ...
  for (const s of sessions) {
    ...
    try {
      if (s.plan.blocks.some((b) => b.rows.some((r) => r.fp === undefined))) unstamped = true
...
export function selectReview(history: RowHistory, newRow: number | null): number[] {
  ...
  if (newRow !== null) {
    ...
    if (nearest) picks.push(nearest.row)
  }
  ...
  if (mostMissed) picks.push(mostMissed.row)
...
export function planSession(
  ...
  if (unfirm.length === rows.length) {
    ...
    const reviewable: RowHistory = new Map(
      [...history].filter(([row]) => servedSet(lesson, history, row, 'review') !== null),
    )
